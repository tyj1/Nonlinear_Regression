...
def mutation(solution):
    ...
    if mutation_prob <1:
        ...
        while(flagx!=1):
            ...
            Flag = mm.inverse_transform((Temp1))
...
pop_setnum = 0
while(s_init < 8):
    solution = geberate_initpop()

    while(gen_no<max_gen):

        function_values = sess.run(yhat, feed_dict={X: solution}) # (-1, 3)
        ...
        while(len(solution2)!=2*pop_size):
            ...
            c = np.array(crossover(solution[a1,:],solution[b1,:])).reshape(1,2)    #
    ...
    # plt.show()
    a=np.array(function1,dtype='float64').reshape((pop_size,1))
    b=np.array(function2).reshape((pop_size,1))
    c=np.hstack((a,b))
    temp = sess.run(yhat, feed_dict={X: c})  # (-1, 3)
    temp.astype(np.float64)
    d=np.hstack([a,b,temp])

    mm_data = mm.fit_transform(matdata)              #归一化
    result = mm.inverse_transform((d))
    ...
    plt.show()
    #np.savetxt("./test/data/s=5时的优化结果.txt", result,fmt='%f',delimiter=',')

    #text_save('./test/data/优化结果(第7个点).txt', result[6,:])
    s_init =s_init + lens
print("操作完成")
#反归一化
#  待写
