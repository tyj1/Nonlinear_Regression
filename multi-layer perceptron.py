...
hide_size1 = 22 #隐藏神经元个数
hide_size2 = 196 #隐藏神经元个数
#预留每个batch中输入与输出的空间
...
with tf.Session() as sess:
    ...
    for i in range(STEPS):
        ...
        if i%100 ==0:
            ...
            summary_str_1 = sess.run(merge_summary_1, feed_dict=
            {x: x_data[int(len(matdata)*0.7):int(len(matdata)*0.85)],
            y: y_data[int(len(matdata)*0.7):int(len(matdata)*0.85)]})
            writer.add_summary(summary_str_1, i)
            summary_str_2 = sess.run(merge_summary_2, feed_dict=
            {x: x_data[int(len(matdata)*0.85):len(matdata)],
            y: y_data[int(len(matdata)*0.85):len(matdata)]})
            writer.add_summary(summary_str_2, i)
